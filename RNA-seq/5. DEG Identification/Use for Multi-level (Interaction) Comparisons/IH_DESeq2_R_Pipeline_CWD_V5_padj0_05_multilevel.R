...
ddsFullCountTable$species <- relevel(ddsFullCountTable$species, ref="PN")
...
res <- results(dds, name="speciesGREM.treatmentherbivory", lfcThreshold = lfcvalue, alpha = padjvalue)
...
baseA <- counts(dds, normalized=TRUE)[, dds@colData$species == "PN"] 
if (is.vector(baseA)){
